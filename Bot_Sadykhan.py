...
# FSM states
target_states = {'name': State(), 'pharmacy': State(), 'rating': State()}
class Form(StatesGroup):
    name     = State()
    pharmacy = State()
    rating   = State()

# Read checklist Excel
...
def log_csv(ph, nm, ts, sc, mx):
    ...
    with open(LOG_PATH, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        if new: writer.writerow(["Дата","Аптека","ФИО","Баллы","Макс"])
...
async def cmd_start(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        "Введите ваше ФИО для авторизации:", parse_mode=ParseMode.HTML)
...
# send question
def build_keyboard(maxv, step):
    kb = InlineKeyboardBuilder()
    start=0 if maxv==1 else 1
    for i in range(start, maxv+1): kb.button(text=str(i), callback_data=f"score_{i}")
...
async def gen_report(chat_id: int, data):
    ...
    for i,h in enumerate(hdrs,1): ws.cell(row=5,column=i,value=h).font=Font(bold=True)
    r=6; tots=0; totm=0
